...
class helper{
    ...
    constructor()
    {
        ...
        this.string= {
            ...
            emoji: (string) => this.stringEmoji(string),
        }
    ...
    /**
     * @name 渲染Emoji表情
     * @description 将 [emoji: URL] 或 [emoji:URL] 或 [emoji:`URL`] 格式的文本替换为 <img> 标签
     * @param {string} string 需要处理的文本
     * @param {string} emojiClass 图片的class
     * @param {string} emojiStyle 图片的style
     * @returns {string} 处理后的HTML文本
     */
    stringEmoji(string = '', emojiClass = 'emoji-img', emojiStyle = '') {
        if (!string) return ''
        const styleAttr = emojiStyle ? ` style="${emojiStyle}"` : ''
        // 兼容多种格式：[emoji:url] / [emoji: url] / [emoji:`url`] / [emoji: `url`]
        // 先做HTML转义防止XSS，再替换emoji
        let result = String(string)
            ...
            .replace(/"/g, '&quot;')
            .replace(/'/g, '&#39;')
        return result.replace(/\[emoji\s*:\s*`?([^`\]]+?)`?\s*\]/g, (match, url) => {
            const cleanUrl = url.trim()
            if (!cleanUrl) return match
            return `<img src="${cleanUrl}" class="${emojiClass}" alt="emoji"${styleAttr} onerror="this.style.display='none'">`
        })
    }
