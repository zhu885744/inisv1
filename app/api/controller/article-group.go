...
const (
	articleGroupAllowFields = "pid,name,key,description,avatar,json,text"
	articleGroupAllowQuery  = "id"
)

var articleGroupAllowFieldsSlice = []any{"pid", "name", "key", "description", "avatar", "json", "text"}
var articleGroupAllowQuerySlice = []any{"id"}

func (this *ArticleGroup) buildQuery(query *facade.ModelStruct, params map[string]any) *facade.ModelStruct {
	return query.
		IWhere(params["where"]).
		IOr(params["or"]).
		ILike(params["like"]).
...
func (this *ArticleGroup) withTrashOptions(query *facade.ModelStruct, params map[string]any) *facade.ModelStruct {
...
func (this *ArticleGroup) getFromCache(ctx *gin.Context, cacheName string) (any, bool) {
...
func (this *ArticleGroup) setCache(ctx *gin.Context, cacheName string, data any) {
	if this.cache.enable(ctx) {
		go facade.Cache.Set(cacheName, data)
	}
}

func (this *ArticleGroup) processFieldValue(val any) any {
...
func (this *ArticleGroup) one(ctx *gin.Context) {
	...
	for key, val := range params {
		if utils.In.Array(key, articleGroupAllowQuerySlice) {
...
func (this *ArticleGroup) rand(ctx *gin.Context) {
	...
	query := facade.DB.Model(&model.ArticleGroup{}).OnlyTrashed(onlyTrashed).WithTrashed(withTrashed)
...
func (this *ArticleGroup) create(ctx *gin.Context) {
	...
	for key, val := range params {
		if utils.In.Array(key, articleGroupAllowFieldsSlice) {
...
func (this *ArticleGroup) count(ctx *gin.Context) {
	params := this.params(ctx)
	query := this.withTrashOptions(facade.DB.Model(&model.ArticleGroup{}), params)
	query = this.buildQuery(query, params)
	count, _ := query.Count()
	this.json(ctx, count, facade.Lang(ctx, "查询成功！"), 200)
}

func (this *ArticleGroup) aggregateQuery(ctx *gin.Context, aggFunc func(query *facade.ModelStruct, field string) any) (any, string) {
	msg := []string{"无数据！", ""}
	var data any
...
func (this *ArticleGroup) remove(ctx *gin.Context) {
	...
	item := facade.DB.Model(&model.ArticleGroup{})
...
func (this *ArticleGroup) delete(ctx *gin.Context) {
	...
	item := facade.DB.Model(&model.ArticleGroup{}).WithTrashed()
...
func (this *ArticleGroup) restore(ctx *gin.Context) {
	...
	item := facade.DB.Model(&model.ArticleGroup{}).OnlyTrashed().WhereIn("id", ids)
	...
	_, err := facade.DB.Model(&model.ArticleGroup{}).OnlyTrashed().Restore(ids)
